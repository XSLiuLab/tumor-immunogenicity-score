...
## Performance comparison on immunotherapy clinical response prediction
...
The predicted values of gene expression biomarkers (i.e. IFNG, CD8, PDL1) is the average values among all members defined by the original publications. 
...
```{r prep_VanAllen2015, eval=FALSE}
#----------------------------------------------------------
# Data source :
...
library(tidyverse)
uro_cli = read_csv("../data/data_clinical.csv")
uro_counts = read_csv("../data/data_counts.csv")
uro_variants = read_csv("../data/data_variants.csv",
                        col_types = "c??????")
uro_rna = read_csv("../data/data_kallisto.csv")
...
```{r}
# library(tidyverse)
sigList = read_csv("../data/ICBbiomarkerSignature.csv", comment = "#")
...
generateInput = function(df) {
  df %>%
    ...
    mutate_at(vars(-Gene, -Ref), `-`, quote(Ref)) %>% 
    select(-Ref) %>% 
...
#f <- function(num, ref) num - !!ref


```

### Compare prediction ROC and survival of APS, IIS, TMB and TIGS
...
#------------------------------------------
# Hugo 2016
hugo2016_roc1 = roc(info_hugo$Response, info_hugo$nTMB)
hugo2016_roc2 = roc(info_hugo$Response, info_hugo$APS)
hugo2016_roc3 = roc(info_hugo$Response, info_hugo$TIGS)
hugo2016_roc4 = roc(info_hugo$Response, info_hugo$IIS)

ggroc(list(TMB=hugo2016_roc1, TIGS=hugo2016_roc3), legacy.axes = TRUE) + 
    labs(color = "Predictor") + ggpubr::theme_classic2(base_size = 12, base_family = "Arial")

auc(hugo2016_roc1)  # AUC of TMB
auc(hugo2016_roc2)  # AUC of APS
auc(hugo2016_roc3)  # AUC of TIGS
auc(hugo2016_roc4)  # AUC of IIS
...
VanAllen2015_roc1 = roc(info_VanAllen2$Response, info_VanAllen2$nTMB)
VanAllen2015_roc2 = roc(info_VanAllen2$Response, info_VanAllen2$APS)
VanAllen2015_roc3 = roc(info_VanAllen2$Response, info_VanAllen2$TIGS)
VanAllen2015_roc4 = roc(info_VanAllen2$Response, info_VanAllen2$IIS)

ggroc(list(TMB=VanAllen2015_roc1,TIGS=VanAllen2015_roc3), legacy.axes = TRUE) + 
    labs(color = "Predictor") + 
    ggpubr::theme_classic2(base_size = 12, base_family = "Arial")

auc(VanAllen2015_roc1)  # AUC of TMB
auc(VanAllen2015_roc2)  # AUC of APS
auc(VanAllen2015_roc3)  # AUC of TIGS
auc(VanAllen2015_roc4)  # AUC of IIS

#---------------------------------------
# Snyder 2017

Snyder2017_roc1 = roc(info_Snyder$is_benefit, info_Snyder$nTMB)
Snyder2017_roc2 = roc(info_Snyder$is_benefit, info_Snyder$APS)
Snyder2017_roc3 = roc(info_Snyder$is_benefit, info_Snyder$TIGS)
Snyder2017_roc4 = roc(info_Snyder$is_benefit, info_Snyder$IIS)

ggroc(list(TMB=Snyder2017_roc1, TIGS=Snyder2017_roc3), legacy.axes = TRUE) + 
    labs(color = "Predictor") + ggpubr::theme_classic2(base_size = 12, base_family = "Arial")

auc(Snyder2017_roc1)  # AUC of TMB
auc(Snyder2017_roc2)  # AUC of APS
auc(Snyder2017_roc3)  # AUC of TIGS
auc(Snyder2017_roc4)  # AUC of IIS
