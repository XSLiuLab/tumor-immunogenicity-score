...
sapply(1:100, function(i){
  ...
  applyGSVA(apm_gens, group_col = "Cell_type", 
...
```{r}
sigList = ISG_IFNG %>% 
  rename(Signature = Type, 
         Genes = GeneSymbol) %>% # make consistent
...
Scale "IFNG.GS", "ISG.RS" signature expression as origin paper said.

```{r}
rna_hugo_sigExpr = rna_hugo_sigExpr %>% 
  dplyr::mutate(IFNG.GS = scale(IFNG.GS) %>% as.numeric(), 
                ISG.RS = scale(ISG.RS) %>% as.numeric())
rna_VanAllen_sigExpr = rna_VanAllen_sigExpr %>% 
  dplyr::mutate(IFNG.GS = scale(IFNG.GS), 
                ISG.RS = scale(ISG.RS))
rna_Snyder_sigExpr = rna_Snyder_sigExpr %>% 
  dplyr::mutate(IFNG.GS = scale(IFNG.GS), 
                ISG.RS = scale(ISG.RS))
```
...
p_roc_4 = ggroc(list(TIGS=roc_df$TIGS[[2]], APS=roc_df$APS[[2]], PDL1=roc_df$PDL1[[2]],
                     CD8=roc_df$CD8[[2]], IFNG=roc_df$IFNG[[2]], IIS=roc_df$IIS[[2]]), legacy.axes = TRUE) + 
  labs(color = "")  +  
  theme(legend.position = c(0.8,0.4)) 
p_roc_5 = ggroc(list(TIGS=roc_df$TIGS[[1]], APS=roc_df$APS[[1]], PDL1=roc_df$PDL1[[1]],
                     CD8=roc_df$CD8[[1]], IFNG=roc_df$IFNG[[1]], IIS=roc_df$IIS[[1]]), legacy.axes = TRUE) + 
  labs(color = "")  +  
  theme(legend.position = c(0.8,0.4)) 
p_roc_6 = ggroc(list(TIGS=roc_df$TIGS[[3]], APS=roc_df$APS[[3]], PDL1=roc_df$PDL1[[3]],
                     CD8=roc_df$CD8[[3]], IFNG=roc_df$IFNG[[3]], IIS=roc_df$IIS[[3]]), legacy.axes = TRUE) + 
  labs(color = "")  +  
  theme(legend.position = c(0.8,0.4)) 
