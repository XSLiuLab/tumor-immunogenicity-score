...
```{r}
df.gsva.tumor = purrr::reduce(list(df.gsva.tumor, res_APS7.GSVA, res_MHCII.GSVA, PHBR_I.score, PHBR_II.score), 
              left_join)
```
...
pheatmap::pheatmap(cor_mat1, display_numbers = TRUE, 
...
for (i in seq_len(nrow(cor_mat2))) {
  ...
  print(pheatmap::pheatmap(cor_mat2$cor[[i]], display_numbers = TRUE,
...
```{r}
df_TMB = df_os %>% filter(!is.na(IIS), !is.na(TMB_NonsynVariants)) %>% 
...
#------ APM
options(digits = 2)
forest_APM = rbind(c("Project", NA, NA, NA, "fdr", "No."),
...
#----- TMB
forest_TMB = rbind(c("Project", NA, NA, NA, "fdr", "No."),
...
#---------- TIGS
forest_TIGS = rbind(c("Project", NA, NA, NA, "fdr", "No."),
...
lapply(study_list, function(study){
    ...
    maf@data %>% group_by(Tumor_Sample_Barcode) %>% 
...
```{r}
ggpubr::ggboxplot(
  df_b2m, x = "B2M_Status", y = "TMB_NonsynVariants",
...
B2M_expr = RNASeq_pancan %>% 
  ...
  dplyr::select(-sample) %>% as.numeric()
...
```{r}
ggpubr::ggboxplot(
  B2M_expr, x = "B2M_Status", y = "B2M_expr",
  xlab = "B2M mutation status"
) + 
  ggpubr::stat_compare_means(method = "t.test", label.x = 1.3)
```
